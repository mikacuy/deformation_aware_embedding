...
def write_ply_color(points, color, filename, text=True):
    num_points = points.shape[0]
    vertex_color = np.array([(int(color[0]), int(color[1]), int(color[2]))]*num_points,
    ...
    ply = PlyData([PlyElement.describe(vertex_all, 'vertex')], text=text).write(filename)

def write_ply_color_sets(pointclouds, filename, text=True):
    color = [[255,51,51],[255,255,51],[51,51,255],[51,255,51],[255,153,51],[255,51,153],[102,102,255],[102,255,255]]
    num_pcs = pointclouds.shape[0]
    ...
    for i in range(num_pcs):
        for j in range(num_points):
            vertex.append((pointclouds[i,j,0],pointclouds[i,j,1],pointclouds[i,j,2]))
            vertex_color.append((int(color[i][0]),int(color[i][1]),int(color[i][2])))

    vertex = np.array(vertex, dtype=[('x', 'f4'), ('y', 'f4'),('z', 'f4')])
    vertex_color = np.array(vertex_color, dtype=[('red', 'u1'), ('green', 'u1'), ('blue', 'u1')])

    vertex_all = np.empty(num_pcs*num_points, vertex.dtype.descr + vertex_color.dtype.descr)
    for prop in vertex.dtype.names:
        vertex_all[prop] = vertex[prop]
    for prop in vertex_color.dtype.names:
        vertex_all[prop] = vertex_color[prop]
