...
with tf.Graph().as_default():
	...
	for i in range(len(model_names)):
		...
		# print(retrieved_chamfer_distances.shape)
		# print(retrieved_deformed_chamfer_distances.shape)
		# print(retrieved_CD_rank)
		# print(retrieved_deformed_CD_rank)
		# exit()
	...
	mean_cd = np.mean(np.mean(all_deformed_cd, axis=1))
	log_string("Average CD error: "+str(mean_cd))
	log_string(" ")

	print(np.min(all_deformed_cd, axis=1).shape)
	min_cd = np.mean(np.min(all_deformed_cd, axis=1)) 
